...
class Cityscapes(data.Dataset):
    ...
    def __init__(self, root, split='train', mode='fine', transform=None):
        ...
        self.depths_dir = os.path.join(self.root, 'depthHHA', split)
        ...
        if not os.path.isdir(self.images_dir) or not os.path.isdir(self.depths_dir) or not os.path.isdir(self.targets_dir):
            raise RuntimeError('Dataset not found or incomplete. Please make sure all required folders for the'
                               ' specified "split" and "mode" are inside the "root" directory')
        
        for city in os.listdir(self.images_dir):
            img_dir = os.path.join(self.images_dir, city)
            depth_dir = os.path.join(self.depths_dir, city)
            target_dir = os.path.join(self.targets_dir, city)

            for file_name in os.listdir(img_dir):
                self.images.append(os.path.join(img_dir, file_name))
                target_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0],
                                             '{}_labelIds.png'.format('gtFine'))
                self.targets.append(os.path.join(target_dir, target_name))
                depth_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0],
                                             'hha.png')
                self.depths.append(os.path.join(depth_dir, depth_name))
